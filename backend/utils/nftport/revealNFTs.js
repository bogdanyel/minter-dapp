...
async function reveal() {
  ...
  for (const meta of ipfsMetas) {
    ...
    if (!ownedNFTs.includes(itemIndex)) {
      const revealedFilePath = `${basePath}/build/revealed/${itemIndex}.json`;
      try {
        ...
        if (revealedFile.length > 0) {
          ...
          if (revealedFileJson.updateData.response !== "OK") {
            ...
            if (check.includes("Success")) {
              ...
              console.log(
                `Transaction failed or not found, will retry revealing Item #${itemIndex}`
              );
              throw `Transaction failed, will retry revealing Item #${itemIndex}`;
        ...
        if (ok) {
          try {
            ...
            fs.writeFileSync(
              `${basePath}/build/revealed/${itemIndex}.json`,
